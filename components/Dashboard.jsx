...
function Dashboard() {
  ...
  async function handleSetMood(mood) {
    ...
    try {
        // Corrected from `newdata` to `newData`
  ...
  return (
    <div className='flex flex-col flex-1 gap-8 sm:gap-12 md:gap-16'>
      <div className='grid grid-cols-3 bg-indigo-50 text-indigo-500 rounderd-lg p-4 gap-4'>
      ...
      {Object.keys(moods).map((mood,moodIndex)=>{
        return(
          <button onClick={()=>{
            ...
            <p className={'text-indigo-500 text-xs sm:text-base ' + fugaz.className}>{mood}</p>
          </button>
        )
      })}
     </div>
